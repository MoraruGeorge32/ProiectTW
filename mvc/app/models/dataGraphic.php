        ...
        if (count($columnSearchedValues) == 2) {
            ...
            while ($stmt->fetch()) {
                for (; $contor < $year && $contor <= $lastYear; $contor++) {
                    array_push($resultsSet[$nume[0]], 0);
                    array_push($resultsSet[$nume[1]], 0);
                }

                $contor = $year + 1;
                array_push($resultsSet[$nume[0]], $v1);
                array_push($resultsSet[$nume[1]], $v2);
            }
        } else {
            $resultsSet[$nume[0]] = [];
            $resultsSet[$nume[1]] = [];
            ...
            while ($stmt->fetch()) {
                for (; $contor < $year && $contor <= $lastYear; $contor++) {
                    array_push($resultsSet[$nume[0]], 0);
                    array_push($resultsSet[$nume[1]], 0);
                    array_push($resultsSet[$nume[2]], 0);
                }
                $contor = $year + 1;
                array_push($resultsSet[$nume[0]], $v1);
                array_push($resultsSet[$nume[1]], $v2);
                array_push($resultsSet[$nume[2]], $v3);
            }
        }
        $data = [];
